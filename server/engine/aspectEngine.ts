// Natal aspect engine — server-side port of src/engine/aspects.ts.
// Uses natal orbs (8° major aspects). Do NOT confuse with ASPECT_DEFS in
// chartEngine.ts, which carries tight transit orbs (2.4° for major aspects).
...
// Natal orbs — full 8° for major aspects. Distinct from ASPECT_DEFS in
// chartEngine.ts (transit orbs at 0.3x natal, five aspects only).
...
// Inlined from zodiac.ts — two lines, no reason to import across module boundaries.
...
export function detectPatterns(aspects: Aspect[]): AspectPattern[] {
  ...
  for (let i = 0; i < trines.length; i++) {
    for (let j = i + 1; j < trines.length; j++) {
      ...
      if (trines.some(t => hasPair(t, other1, other2))) {
        ...
        if (!patterns.some(p => p.type === 'Grand Trine' && arraysEqual(p.planets.slice().sort(), trio))) {
  ...
  for (const opp of oppositions) {
    for (const sq1 of squares) {
      ...
      if (squares.some(s => hasPair(s, apex, oppOther))) {
        ...
        if (!patterns.some(p => p.type === 'T-Square' && arraysEqual(p.planets.slice().sort(), trio))) {
  ...
  // NOTE: hasAllSquares guard is copied faithfully from aspects.ts — known weak
  // enforcement (short-circuits on first match). Behavioral parity is intentional.
  ...
  for (let i = 0; i < quincunxes.length; i++) {
    for (let j = i + 1; j < quincunxes.length; j++) {
      ...
      if (sextiles.some(s => hasPair(s, other1, other2))) {
        const trio = [shared, other1, other2].sort()
        if (!patterns.some(p => p.type === 'Yod' && arraysEqual(p.planets.slice().sort(), trio))) {
