...
export interface TransitPosition extends ZodiacPosition {
  name: string
  retrograde: boolean
  house: number
...
export interface TransitData {
  ...
  retrogrades: { planet: string; isRetro: boolean; status: string }[]
}
...
export function calculateTransitAspects(
  ...
  natalPlanets: Array<{ name: string; longitude: number; house: number; sign: string }>,
  period: TransitPeriod,
  unknownTime = false,
): TransitAspect[] {
 ...
 */
export function getRetrogradeStatus(date: Date): { planet: string; isRetro: boolean; status: string }[] {
  const time = Astronomy.MakeTime(date)
  const statuses: { planet: string; isRetro: boolean; status: string }[] = []
...
export function assignTransitHouses(
  transitPlanets: TransitPosition[],
  natalHouses: Array<{ house: number; longitude: number }>,
): TransitPosition[] {
