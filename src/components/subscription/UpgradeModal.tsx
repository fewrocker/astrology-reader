...
export default function UpgradeModal({
  ...
  // useRef instead of useState so the value is available synchronously in the catch block,
  // avoiding the bug where ceremonyStartedAt reads stale state (0) on fast network failures.
  const ceremonyStartedAt = useRef<number>(0)
  ...
  const handleCheckout = async (priceId: string, tier: 'basic' | 'advanced') => {
    ...
    ceremonyStartedAt.current = start
    ...
    try {
      ...
      const elapsed = Date.now() - ceremonyStartedAt.current
