...
import type { BodyName, ZodiacSign } from '../../engine/types'
...
function EventCard({ event, expanded, onToggle }: { event: TimelineEvent; expanded: boolean; onToggle: () => void }) {
  ...
  if (event.type === 'aspect-perfection' && event.aspectType && event.secondPlanet) {
    ...
    detailText = getIngressBrief(event.planet as import('../../engine/types').PlanetName, event.toSign)
    ...
    detailText = getStationBrief(event.planet as import('../../engine/types').PlanetName, event.stationType)
  }
