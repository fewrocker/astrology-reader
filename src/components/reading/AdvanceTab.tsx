...
function buildPowerReason(
  ...
  return { reason: `${planet} ${verb} ${angleName} — ${domain}.` }
}
...
function buildAspectReason(
  ...
  if (category === 'favorable') {
    return { reason: `${planet} ${type} your natal ${natalPlanet} — a window of ${domain}.` }
  } else {
    return { reason: `${planet} ${type} your natal ${natalPlanet} — tension around ${domain}.` }
  }
}
...
function scoreSnapshot(
  ...
  if (!chartData.unknownTime) {
    ...
    if (bestContact) {
      ...
      const { reason, guidance } = buildPowerReason(bestContact.planet, bestContact.aspectType, bestContact.angleKey)
  ...
  if (stationPlanet && stationDirection) {
    ...
    if (isFavorable || isChallenging) {
      ...
      const { reason: coReason, guidance: coGuidance } = buildAspectReason(tightest, primaryCategory)
  ...
  // ── Priority 3: favorable ─────────────────────────────────────────────────
  {
    ...
    if (rating.score >= 4 && tightApplyingHarmonious.length >= orbs.energyMinAspects) {
      ...
      const { reason, guidance } = buildAspectReason(tightest, 'favorable')
  ...
  // ── Priority 4: challenging ───────────────────────────────────────────────
  {
    ...
    if (rating.score <= 2 && tightApplyingChallenging.length >= orbs.energyMinAspects) {
      ...
      const intensity = Math.abs(rating.score - 3) / 2
      const { reason, guidance } = buildAspectReason(tightest, 'challenging')
