...
**Status:** Done — commit `43429ee` on branch `sprint-0001-task-0004-feat-numerology-reading`

**Delivered:**
- `src/engine/numerology.ts` — full calculation engine with `reduceToSingleDigit` (preserving master numbers 11, 22, 33), `calculateLifePath`, `calculateBirthdayNumber`, `calculatePersonalYear`, `calculateExpressionNumber`, and the top-level `calculateNumerology` function.
- `src/data/numerologyInterpretations.ts` — rich authentic interpretations for numbers 1–9 + 11, 22, 33 across all four categories (lifePath, birthdayNumber, personalYear, expressionNumber), each with archetype, 3-4 sentence essence, shadow, and 5-7 keywords.
- `src/components/results/NumerologyPage.tsx` — beautiful page matching the mystic dark theme: large glowing golden number display, archetype titles, essence text, keyword pills, expandable shadow section, optional name input for Expression Number, and a "Cosmic Connections" section with dynamic natal chart cross-references keyed to actual planetary placements (Sun, Moon, Mercury, Venus, Mars, Jupiter, Saturn, Neptune, Pluto) and house positions.
- `src/context/appState.ts` — `userName?: string` added to `BirthData`, `SET_USER_NAME` action type and reducer case, and localStorage persistence via the existing `saveBirthData` flow.
- `src/App.tsx` — `'numerology'` added to `AppView` union, `NumerologyPage` imported and rendered, and a gold-accented "Numerology ✦" button added to `CachedDataLanding` between Dream Interpretation and Enter New Birth Data.

**Build:** TypeScript type-check and `vite build` both pass with zero errors.
