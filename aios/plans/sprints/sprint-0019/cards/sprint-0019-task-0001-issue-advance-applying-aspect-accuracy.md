...
**Commit:** `047abe0` — `fix(sprint-0019): advance applying-aspect accuracy — filter separating aspects from energy gate, stabilize applying flag at stations`

**Bug 1 fix:** In `AdvanceTab.tsx`, all three `computeEnergyRating(snapshot.transitAspects)` calls in the Priority 2, 3, and 4 gate blocks were replaced with `computeEnergyRating(snapshot.transitAspects.filter(a => a.applying))`. The function itself (`transits.ts:498–515`) was not modified — it continues to serve its other call sites correctly. The energy gate now only counts forward-looking momentum.

**Bug 2 fix:** In `calculateTransitAspects` (`transits.ts:172–174`), added a station check before the applying formula: `const stationThreshold = STATION_THRESHOLD[tp.name as PlanetName]; const isStationing = stationThreshold !== undefined && Math.abs(tp.dailyMotion) < stationThreshold`. When stationing, `applying` is forced to `true` rather than derived from the unreliable sign of `dailyMotion`. Non-planet bodies (NorthNode, asteroids) are unaffected since they have no entry in `STATION_THRESHOLD`.

Build: TypeScript clean, Vite production build successful (1888 modules).
