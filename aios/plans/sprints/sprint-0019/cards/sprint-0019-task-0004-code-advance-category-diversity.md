...
Replaced the pure-intensity density cap in `preCalculateSnapshots` (AdvanceTab.tsx, lines 491–525) with a two-phase category-aware selection:

- **Phase 1:** iterates `['power', 'favorable', 'challenging', 'shift']`, finds the highest-intensity marker per category, and places it in `reservedOffsets`.
- **Phase 2:** sorts all non-reserved non-neutral markers by intensity descending, takes `maxMarkers - reservedOffsets.size` more to fill capacity.

The final keep-set is the union; all other non-neutral snapshots are neutralized. `OverviewStrip` and `MarkerDot` required no changes. Build passed with no TypeScript errors.

Commit: `beed7ea` on branch `sprint-0019-task-0004-code-advance-category-diversity`.
