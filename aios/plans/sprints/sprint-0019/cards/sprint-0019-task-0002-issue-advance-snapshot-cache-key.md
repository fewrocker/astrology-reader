...
**Status:** completed
**Commit:** a79648d
**Branch:** sprint-0019-task-0002-issue-advance-snapshot-cache-key

`ChartData` has no `birthDate`/`birthTime` fields — those live on `BirthData`. Instead, the chart identity fingerprint was derived from fields available directly on `ChartData`: `angles.ascendant.longitude`, `angles.midheaven.longitude`, and `unknownTime`. The ascendant longitude encodes both birth date and time (it completes a full 360° rotation in ~24 hours), making it a compact and accurate per-chart identity token.
...
to:
```
${asc.toFixed(4)}:${mc.toFixed(4)}:${unknownTime}:${period}:${baseDate.toISOString()}
```

The `toFixed(4)` precision (~0.0001° ≈ 0.36 arcseconds) gives a stable string representation that won't produce spurious misses from floating-point noise in repeated computations of the same chart, while still being unique per person.
