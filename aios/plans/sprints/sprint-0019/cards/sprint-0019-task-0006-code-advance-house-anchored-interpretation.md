...
**Completed:** 2026-05-16 — commit `987dcd9`

**What was built:**

1. **`TRANSIT_PLANET_PHRASES` exported** from `transitAspectBriefs.ts` so the verb table (10 planets × 3 natures × applying/separating) can be shared with the scoring layer.

2. **`buildAspectReason()` rewritten** — now uses `TRANSIT_PLANET_PHRASES` for the verb phrase, `getHouseTheme()` for the house, and a `ASPECT_HOUSE_CONTEXT` lookup table (planet × nature × house 1–12) for the reason suffix. Example output: `"Saturn pressing on your Moon in your 7th house (partnership) — a committed relationship is being tested structurally — patterns that have gone unexamined are now surfacing."` Falls back gracefully when house is null or planet unrecognized.

3. **`ASPECT_HOUSE_CONTEXT` table** — keyed `"${planet}|${nature}|${house}"`, covering Saturn/Jupiter/Pluto/Uranus/Neptune × harmonious/challenging × all 12 houses. Each entry is a punchy, specific, house-aware reason suffix.

4. **`buildPowerReason()` rewritten** — keyed `"${planet}|${aspectType}|${angle}"` against `POWER_DAY_PHRASES` table. 40 entries covering Saturn/Jupiter/Pluto/Uranus × all six aspect types × ASC/MC. Each entry has a specific `reason` and `guidance` rather than the previous static two-phrase fallback.

5. **`guidance?: string` added to `SnapshotScore` interface** — populated by both build functions. Propagated through all four `scoreSnapshot` return sites.

6. **`ASPECT_GUIDANCE` table** — keyed `"${planet}|${nature}"`, 17 entries covering all major transit planets × harmonious/challenging. One navigational sentence per combination (e.g., Saturn challenging: "Face the pattern directly rather than managing around it…").

7. **Banner render updated** — the category banner now wraps reason and guidance in a `<div>`. Guidance renders as `text-xs text-mystic-muted/80 font-light leading-relaxed` beneath the reason line. `MarkerTooltip` unchanged — guidance intentionally excluded per card spec.

**Not built:** Shift marker improvement (nearest natal planet proximity + TRANSIT_RETROGRADE brief) — out of scope per "What to build" items; station marker text remains `"${planet} stations ${direction}."`.

Build: clean, no TypeScript errors. 401 net insertions.
...
**`buildAspectReason()` — house-anchored, verb-table-driven:**
- Imports `TRANSIT_PLANET_PHRASES` (now exported from `transitAspectBriefs.ts`) and `getHouseTheme` from `houseThemes.ts`
- Uses `tightest.natalHouse` + `tightest.nature` + `tightest.applying` to select the archetype verb phrase
- Builds reason as: `"${planet} ${verbPhrase} your ${natalPlanet} in your ${ordHouse} house (${houseShortName}) — ${contextPhrase}"`
- `ASPECT_HOUSE_CONTEXT` table: 76 entries keyed `"${planet}|${nature}|${house}"` — covers Saturn/Jupiter/Pluto/Uranus/Neptune × harmonious|challenging × all 12 houses, with specific one-sentence descriptions per combination
- Falls back to `"${houseTheme.theme.toLowerCase()} themes are [supported/under pressure]."` for uncovered combinations
- Returns `{ reason, guidance }` not bare `string`

**`buildPowerReason()` — planet-specific angle phrases:**
- `POWER_DAY_PHRASES` table: 40 entries keyed `"${planet}|${aspectType}|${angle}"` — covers Saturn/Jupiter/Pluto/Uranus × all 5 aspect types × ASC|MC
- Returns `{ reason, guidance }` with angle- and planet-specific language; falls back to original verb+domain pattern

**`guidance?: string` field on `SnapshotScore`:**
- `ASPECT_GUIDANCE` table: keyed `"${planet}|${nature}"` — 20 entries covering all major transit planets with navigational action language (favorable: what to initiate; challenging: what posture helps)
- Populated by both `buildAspectReason` and `buildPowerReason` and spread into all 4 return paths in `scoreSnapshot`
- Banner render updated: guidance appears as a second `<p>` below the reason in `text-xs text-mystic-muted/80 font-light leading-relaxed` — only when `snapshot.score.guidance` is present
- `MarkerTooltip` unchanged — guidance not shown in tooltip

### Sample output (vs previous)

**Before:** `"Saturn opposition your natal Moon — tension around structure and discipline."`
**After reason:** `"Saturn pressing on your Moon in your 7th house (partnership) — a committed relationship or important one-on-one dynamic is being examined for durability — what isn't working can't be deferred."`
**After guidance:** `"Face the pattern directly rather than managing around it — what gets examined and restructured now builds a foundation that actually holds."`

**Before:** `"Saturn reaches your Ascendant — a significant moment for identity and how the world first meets you."`
**After reason:** `"Saturn arrives at your Ascendant — the way you carry yourself in the world is being restructured, and what others have assumed about you may not hold."`
**After guidance:** `"Let the old presentation go deliberately rather than waiting for it to be stripped away — what you build in its place will be more authentically yours."`
