...
Installed `helmet`, `compression`, and `morgan` (plus `@types/compression` and `@types/morgan`). Updated `server/index.ts` with all six hardening layers:

- `helmet()` middleware before all routes for security headers
- `compression()` middleware before routes for gzip API responses
- `morgan('dev'|'combined')` request logging (dev vs production)
- `express.json({ limit: '50kb' })` with a comment warning that Stripe webhook routes (feat-stripe-checkout) must be mounted before this middleware using `express.raw()`
- Global async error handler after all routes, returning `{ error: 'internal_error' }` with no stack trace leak
- Production env guards for `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` (process.exit(1) if missing in production)
- Improved ephemeral `JWT_SECRET` warning mentioning subscription/session impact
