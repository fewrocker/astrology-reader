...
**Branch:** sprint-0009-task-0002-feat-funnel-analytics

### What was implemented

**Server-side:**
- `cookie-parser` installed; session cookie middleware in `server/index.ts` sets a `session_id` UUID cookie (HttpOnly, SameSite=Lax, 1-year expiry) on all `/api/*` requests
- `events` table added to `server/db.ts` with 3 indexes (`idx_events_session_event`, `idx_events_user_event`, `idx_events_created_at`) created at table-creation time
- 90-day purge interval in `server/index.ts` (runs every 24 hours, errors logged but non-fatal)
- `server/routes/analytics.ts` — `POST /api/analytics/event` with input validation (max 64 chars), best-effort JWT userId resolution, always returns 200

**Client-side:**
- `src/services/analytics.ts` — fire-and-forget `track(event, properties?)` using `AUTH_TOKEN_KEY = 'astral-auth-token'` (the correct key used by authService.ts and AuthContext.tsx)

**10 events wired:**
1. `page_view` — `App.tsx` AppContent mount, with `{ view, has_cached_data }`
2. `form_started` — `FormWizard.tsx` first Next click, via ref guard
3. `form_completed` — `App.tsx` after `SET_RESULTS` dispatch, with `{ has_birth_time }`
4. `reading_viewed` — 8 result pages on mount: natal, transit, synastry, synastry_transit, solar_return, numerology, today, journal
5. `gpt_request_made` — `gptInterpretation.ts` callProxy after success, with `{ gpt_type }`
6. `gpt_limit_hit` — `gptInterpretation.ts` callProxy on 429, with `{ authenticated, gpt_type }`
7. `auth_nudge_seen` — `HomeScreen.tsx` IntersectionObserver on nudge button ref, fires once
8. `auth_nudge_clicked` — `HomeScreen.tsx` nudge button onClick, before `onOpenAuth()`
9. `login_completed` — `AuthContext.tsx` login callback, with `{ method: 'email' }`
10. `signup_completed` — `AuthContext.tsx` register callback, with `{ method: 'email' }`

**Skipped (belong to feat-stripe-checkout):** `upgrade_modal_shown`, `upgrade_modal_dismissed`, `upgrade_modal_tier_hovered`, `checkout_started`

### Build result
`npm run build` passed with 0 TypeScript errors. Chunk size warnings are pre-existing (city database), not introduced by this task.
