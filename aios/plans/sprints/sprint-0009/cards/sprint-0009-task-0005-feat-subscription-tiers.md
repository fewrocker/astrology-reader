...
**Status:** done
**Branch:** sprint-0009-task-0005-feat-subscription-tiers
**Completed:** 2026-05-14

### What was implemented

**Part A — DB Migrations (`server/db.ts`):**
- `ALTER TABLE users ADD COLUMN IF NOT EXISTS subscription_tier TEXT DEFAULT 'free'` — idempotent, requires SQLite 3.35+ with clear startup error if version is too old
- `ALTER TABLE users ADD COLUMN IF NOT EXISTS stripe_customer_id TEXT`
- New `gpt_usage (user_id, date, count)` table with `PRIMARY KEY (user_id, date)` and `ON DELETE CASCADE`

**Part B — Tier-aware rate limiter (`server/middleware/gptRateLimit.ts`):**
- `RateLimitEntry` gains `tier` field; `TIER_LIMITS = { free:3, basic:20, advanced:100 }`
- Prepared statements cached at module level for hot-path performance
- Entry creation: one synchronous DB read for tier + one for today's usage count (restart-safe counter restore)
- Atomic `INSERT...ON CONFLICT...DO UPDATE SET count = count+1 WHERE count < limit` — eliminates the check-then-act race
- `releaseSlot` callback attached to `res.locals` before `next()`; decrements both in-memory and DB counters
- `invalidateUserRateLimit(userId)` exported for Stripe webhook use
- Localhost bypass gated on `NODE_ENV !== 'production'`
- `res.locals.userId` populated for downstream feature gates
- `declare module 'express-serve-static-core'` augmentation for `releaseSlot` and `userId`

**Part C — GPT route (`server/routes/gpt.ts`):**
- `cosmic-pattern-reading` gate: checks `res.locals.userId` and DB tier, returns 403 with `releaseSlot()` call before returning
- `releaseSlot?.()` called on `gpt_unavailable` (503) and unexpected 500; NOT called on 4xx

**Part D — Auth route (`server/routes/auth.ts`):**
- `UserRow` interface gains `subscription_tier` and `stripe_customer_id`
- `safeUser()` returns `subscriptionTier` with `?? 'free'` guard for pre-migration rows
- All SELECT statements updated to include new columns via `SELECT_USER_COLS` constant
- `GET /api/auth/usage` endpoint returns `{ todayUsed, limit, tier, resetAt }`

**Part E — `authService.ts`:**
- `AuthUser.tier: 'free' | 'basic' | 'advanced'`
- `ServerUserProfile.subscriptionTier` field
- `UsageResponse` interface + `getUsage()` function

**Part F — `gptErrors.ts`:**
- `GptRateLimitError` interface + `parseRateLimitError()` function
- Legacy string constants preserved for backward compatibility

**Part G — `AuthContext.tsx`:**
- `AuthContextType` gains `tier` and `todayUsed`
- Session restore populates both (with `getUsage()` call after login/session-fetch)
- `payment=success` URL param triggers session re-fetch then URL cleanup
- `just-registered` sessionStorage flag set on register

**Part H — `SessionBadge` (`src/App.tsx`):**
- Destructures `tier` and `todayUsed` from `useAuth()`
- Free tier: shows "N readings left today" in `rgba(201,168,76,0.45)` with `role="status" aria-live="polite"`
- Basic/Advanced: shows "Basic ✦" / "Advanced ✦" in `rgba(201,168,76,0.75)`

**Part I — `HomeScreen.tsx`:**
- Six-state usage-aware nudge (unauthenticated 0-1/2/3+, authenticated free 0-1/2/3+, paid)
- First-visit welcome sentence via `just-registered` sessionStorage flag
- Proper `aria-label` attributes on all nudge buttons

### Open question resolutions
- Q1: Separate `/api/auth/usage` endpoint chosen (cleaner separation of concerns)
- Q5: Unauthenticated nudge uses `todayUsed` from context (defaults to 0 for unauthenticated — shows "Save your readings ✦" copy at 0-1 which is the correct default experience)
- Q6: `sessionStorage` approach used for welcome sentence

### Build
TypeScript + Vite build: clean (0 errors)
