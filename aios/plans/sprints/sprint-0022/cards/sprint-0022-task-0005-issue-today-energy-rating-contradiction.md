...
Changed the Transit Energy card render guard in `src/components/reading/TodayPage.tsx` (line 252) from:
...
The comment was also updated to document the suppression condition. Build passed cleanly. The `computeEnergyRating` computation is preserved as a fallback for days when no advance score is available or the score is neutral.
