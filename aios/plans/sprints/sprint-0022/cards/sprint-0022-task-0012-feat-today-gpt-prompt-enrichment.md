...
**Status:** completed
**Commit:** `bda4685` — feat(sprint-0022): today GPT prompt enrichment — natal chart, essence, advance signal, house context

### Key changes

**`src/components/reading/TodayPage.tsx`**
- Captured advance score in `localAdvanceScore` local variable (in addition to `setAdvanceScore`) to avoid reading stale React state at the GPT call site.
- Derived natal Big Three: `natalSunSign`, `natalMoonSign`, `natalAscSign` from `chartData.planets` and `chartData.angles.ascendant`.
- Derived `natalMoonHouse` with `|| null` to treat house 0 (unknownTime) as null.
- Derived `natalMoonPhase` from natal Sun-Moon elongation using 8-bucket boundary logic (New Moon through Waning Crescent); null-safe if either longitude is undefined.
- Built `aspectBriefSentences` by calling `computeTransitAspectBrief` for each transit in `top[]` — the same call already used by `AspectRow`.
- Built `enrichedAspects` with `natalHouse` and `applying` included.
- Extended `getTodayPageInterpretation` call with all 9 new arguments.

**`src/services/gptInterpretation.ts`**
- Extended `getTodayPageInterpretation` signature with 9 new positional parameters (chose positional for continuity with existing style per open question 1).
- Extended `callProxy` payload to include all new fields (including explicit nulls for optional fields).
- Enriched `aspects` array type to include `natalHouse` and `applying`.

**`server/services/gpt.ts`**
- Extended `handleTodaySynthesis` payload type with 9 optional enrichment fields.
- Rebuilt prompt into 4 named sections joined by `\n\n`: Natal Chart (with ordinal house suffix), Personal Day (with essence first sentence), Today's Sky (using `aspectBriefSentences` with raw fallback), Advance Signal (omitted when null).
- Added `ordinalSuffix()` helper for house number formatting (1st, 2nd, 3rd, 4th–13th handled correctly).
- Replaced generic closing instruction with synthesis-not-enumeration directive per spec 12.
- Retained `max_tokens: 350`, `temperature: 0.8`, and server-side personal day cross-check unchanged.
