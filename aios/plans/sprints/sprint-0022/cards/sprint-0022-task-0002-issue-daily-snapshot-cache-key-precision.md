...
**Commit:** `c300e08` — `fix(sprint-0022): daily snapshot cache key precision — toFixed(4) + ascendant + moon sign`

**Changes:**
- `src/components/reading/DailySnapshotCard.tsx` — `getCacheKey` now resolves `moon` and `asc` from `chart` and constructs the key as `${CACHE_PREFIX}${sun?.longitude?.toFixed(4)}-${asc?.sign ?? 'unknown'}-${moon?.sign ?? 'unknown'}-${today}`, matching the precision and discriminator pattern established by the advance cache fix.

**Build:** clean (tsc + vite, no type errors).
