...
**Branch:** sprint-0022-task-0011-feat-synastry-aspect-house-note  
**Commit:** 2a43327

Implemented as specified. Two files changed (76 insertions, 19 deletions):

- `src/components/reading/AspectRow.tsx`: Added optional `expansionNote?: string` prop. When present, renders `<p className="text-mystic-muted/70 text-xs leading-relaxed mt-1.5">` beneath the brief inside the expansion panel. `maxHeight` increases from `6rem` to `8rem` when `expansionNote` is truthy.
- `src/components/results/SynastryPage.tsx`: Extended `HouseOverlay` import from `../../engine/synastry`. Added `QUALIFYING_HOUSES = [1, 5, 7, 8]` constant. Added pure `resolveHouseNote` function with guard for invalid houses (`entry.house > 0 && entry.house <= 12`), both direction checks (A: person1Planet in person2's houses; B: person2Planet in person1's houses), and tie-breaking by `QUALIFYING_HOUSES` index order (Check A preferred on equal house). Updated `SynastryAspectsSection` props to include optional `houseOverlay?: HouseOverlay`, computed `houseNote` per aspect row, and passes `expansionNote={houseNote ?? undefined}` to `AspectRow`. Call site at line 497 now passes `houseOverlay={synastryData.houseOverlay}`.

Open question 3 resolution: used `theme.theme.toLowerCase()` for the parenthetical — descriptive and consistent with HouseOverlaySection's own note construction. Build passed with zero TypeScript errors.
