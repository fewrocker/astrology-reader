...
### 1. Component structure
- [x] 1.1 Bi-wheel implemented by extending `ChartWheel` with new props (no new top-level component)
- [x] 1.2 `synastryPlanets` typed as `PlanetPosition[]` — no `dailyMotion` accessed
- [x] 1.3 `synastryAspects` typed as `SynastryAspect[]`, rendered directly, hover tracked by index
- [x] 1.4 Synastry mode takes precedence: `isSynastryMode` gate prevents transit ring when synastry props present
- [x] 1.5 `SolarReturnBiWheel.tsx` not modified or referenced

### 2. Outer ring placement and geometry
- [x] 2.1 `SYNASTRY_PLANET_R = 288` (alias to `TRANSIT_PLANET_R`)
- [x] 2.2 `offset(p.longitude)` used for Person 2's planets — same reference frame as natal
- [x] 2.3 No radial nudge applied to synastry planets
- [x] 2.4 Divider circle at r=276 drawn when synastry mode active

### 3. Person 2 planet visual style
- [x] 3.1 Color `#c084fc` (lilac-purple) — distinct from natal gold and transit teal
- [x] 3.2 `synastryGlow` filter added to `<defs>` using SYNASTRY_COLOR flood
- [x] 3.3 `r={11}` at rest, `r={14}` on hover, transparent `r={16}` touch target
- [x] 3.4 Superscript "2" marker at +10, -8 from planet center
- [x] 3.5 Retrograde indicator rendered for retrograde synastry planets
- [x] 3.6 All planets in `synastryPlanets` rendered (no filtering by body type)

### 4. Cross-aspect line rendering
- [x] 4.1 Lines visible at rest (no hover gate)
- [x] 4.2 Base opacity 0.30 (within 0.25–0.35 range)
- [x] 4.3 `strokeDasharray="6 4"` dashed strokes
- [x] 4.4 Rest weight 0.6–0.9px, hover 2.2px
- [x] 4.5 Endpoints: natal body at PLANET_R or ASTEROID_R (isAsteroid guard), synastry body at SYNASTRY_PLANET_R
- [x] 4.6 Color: `#4a7fb5` harmonious, `#b54a4a` challenging, `#c9a84c` neutral
- [x] 4.7 Major-type filter applied via `filteredSynastryAspects` useMemo
- [x] 4.8 Transparent strokeWidth="12" hit-target line on each aspect
- [x] 4.9 Hovered line opacity increases to 1.0, stroke changes to light
- [x] 4.10 Hovering Person 2 planet dims unconnected lines, brightens connected ones

### 5. HoverState extensions
- [x] 5.1 `{ kind: 'synastry'; name: string }` and `{ kind: 'synastryAspect'; index: number }` added
- [x] 5.2 `handleTap` equality check updated for both new variants
- [x] 5.3 `tooltipBorderColor` handles `synastry` (SYNASTRY_COLOR) and `synastryAspect` (aspect nature color)
- [x] 5.4 Synastry hover does not conflict with `transit` kind

### 6. Tooltip components
- [x] 6.1 `SynastryPlanetTooltip` — shows glyph, name in SYNASTRY_COLOR, "Partner" badge, degree/sign, cross-aspects list
- [x] 6.2 `SynastryAspectTooltip` — shows P1 glyph (gold), aspect symbol (nature color), P2 glyph (purple), aspect name, orb, nature, synastry brief
- [x] 6.3 `TransitPlanetTooltip` and `TransitAspectTooltip` not used for synastry hover states
- [x] 6.4 Both tooltips rendered in desktop hover div and mobile bottom-sheet modal
- [x] 6.5 `tooltipH` raised to 300 for synastry tooltip variants in `tooltipStyle`

### 7. SynastryPage layout changes
- [x] 7.1 Side-by-side `grid grid-cols-2` replaced with single centered bi-wheel container
- [x] 7.2 Container uses `max-w-2xl`
- [x] 7.3 `ChartWheel` receives `chartData`, `aspects`, `synastryPlanets`, `synastryAspects`
- [x] 7.4 Label above bi-wheel uses birth dates (most personal available identifier; names not yet collected)
- [x] 7.5 `IndividualChartSection` components use `CollapsibleSection` without `defaultOpen` (collapsed by default)
- [x] 7.6 `CurrentMoonWidget` removed from synastry page
- [x] 7.7 `SynastryAspectsSection` changed from `defaultOpen` to collapsed by default

### 8. SVG accessibility
- [x] 8.1 `aria-label` updated to "Synastry bi-wheel chart" when `isSynastryMode`
- [x] 8.2 Each Person 2 planet `<g>` has `aria-label="{name} in {sign} — Partner planet"`

### 9. Coordinate system correctness
- [x] 9.1 `offset(lon)` using Person 1's ASC longitude applied to all rings
- [x] 9.2 No special guard needed for unknown time (pre-existing behavior)
- [x] 9.3 `isAsteroid` guard used to select natal body radius for line endpoints

### 10. Mobile and responsive behavior
- [x] 10.1 `viewBox="0 0 700 700"` with `w-full` — scales correctly
- [x] 10.2 Transparent `r={16}` touch targets on outer-ring planets
- [x] 10.3 Tapping planet/aspect opens bottom-sheet modal with correct content

### 11. Edge cases
- [x] 11.1 Identical birth data — no special handling needed, geometry is correct
- [x] 11.2 Empty `synastryAspects` — outer ring renders, no lines drawn
- [x] 11.3 Unknown birth time — no new guard needed
- [x] 11.4 All aspects one nature — renders correct single-color lines
- [x] 11.5 Subset of bodies — absent bodies simply not rendered
- [x] 11.6 Near-zero length lines — rendered correctly as expected

### 12. Performance constraints
- [x] 12.1 Tick mark count unchanged
- [x] 12.2 At most ~45 synastry line elements added
- [x] 12.3 `filteredSynastryAspects` via `useMemo` keyed on prop
- [x] 12.4 Synastry tooltips rendered conditionally inside `hover.kind === 'synastry'` / `hover.kind === 'synastryAspect'` guards

### 13. Type system integrity
- [x] 13.1 `synastryPlanets` typed as `PlanetPosition[]` from `../../engine/types`
- [x] 13.2 `synastryAspects` typed as `SynastryAspect[]` from `../../engine/synastry`
- [x] 13.3 New HoverState variants exhaustively handled in handleTap, tooltipBorderColor, and tooltip renders
- [x] 13.4 `computeSynastryAspectBrief` imported and called with correct signature in `SynastryAspectTooltip`

### 14. Acceptance checks
- [x] 14.1 Cross-aspect lines rendered connecting inner-ring planets to outer-ring partners
- [x] 14.2 No radial nudge — conjunction renders at correct position
- [x] 14.3 Lines visible at initial page load without user interaction
- [x] 14.4 Transit reading unaffected (synastry props absent = transit mode unchanged)
- [x] 14.5 Natal reading unaffected (no outer ring props = no outer ring rendered)
- [x] 14.6 Mobile: touch targets at r=16, bottom-sheet opens on tap
- [x] 14.7 No "Transit" labeling in synastry tooltips — "Partner" badge used instead
